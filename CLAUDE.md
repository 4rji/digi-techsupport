...
| `digi-remote-service.js` | Main + HTTP server | Digi Remote Manager (DRM) integration. `getDevices()` fetches `/ws/v1/devices/inventory` and maps to `{ id, name, status }`. `getDeviceDetail`/`getDeviceEvents`/`getDeviceAlerts` fetch per-device data (v1 REST); `queryDeviceState`/`rebootDevice`/`runDeviceCli` use SCI (`POST /ws/sci`, XML, reuses the same API key — `runDeviceCli` sends one `<cli><execute>` command and parses the reply output; request/response, not an interactive shell); `getDeviceLogs` returns log text for the Support Archive Viewer. All share a `drmRequest` helper. Credential read/write helpers store `digi-remote-credentials.json` (mode 0o600) in userData. Throws `ConfigurationError` (no key) / `AuthError` (401) / `NotFoundError` (404). Never logs or returns the secret. |
...
- `digi-run-device-cli` — runs one CLI command on a device via SCI (`<cli><execute>`) and returns the output; backs the DRM device-detail Console pane (command → output history, works behind NAT). Not exposed over HTTP (arbitrary command execution).
